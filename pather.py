...
class PathSolve():
    ...
    def shortest_path_fruit(self):
        #用曼哈顿距离蛇头判断最近的水果
        print(self.snake.snakebody[0])
        dis_fruit0 = self.manhattan_distance(self.snake.snakebody[0],self.fruit.list_generate[0])
        dis_fruit1 = self.manhattan_distance(self.snake.snakebody[0],self.fruit.list_generate[1])
        if (dis_fruit0 <= dis_fruit1):
            self.fruit.shortestfruit = self.fruit.list_generate[0]
        else:
            self.fruit.shortestfruit = self.fruit.list_generate[1]
        return self.shortest_path(self.fruit.shortestfruit)
    def shortest_fruit(self):
        #用曼哈顿距离蛇头判断最近的水果
        print(self.snake.snakebody[0])
        dis_fruit0 = self.manhattan_distance(self.snake.snakebody[0],self.fruit.list_generate[0])
        dis_fruit1 = self.manhattan_distance(self.snake.snakebody[0],self.fruit.list_generate[1])
        if (dis_fruit0 <= dis_fruit1):
            ...
            self.fruit.shortestfruit = self.fruit.list_generate[1]
        return self.fruit.shortestfruit
    def manhattan_distance(self, point1, point2):
        distance = int(np.abs(point1[0] - point2[0]) + np.abs(point1[1] - point2[1]))
        return distance
