...
class FruitDrawer(Drawer):
    ...
    def remove(self,point):
        x = point[0]
        y = point[1]
        print("removvvvvvvvvvvvvvvvvvvvvvvve")
        print(point)
        self.fruit.remove_fruit(point)
        self._basic_draw(x, y, WHITE)
        print("whiteeeeeeeeeeeeee")
    def draw(self):
        for i in range(len(self.fruit.list_generate)):
            (x, y) = self.fruit.list_generate[i-1]
